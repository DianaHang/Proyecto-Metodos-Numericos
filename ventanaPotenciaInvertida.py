...
from potenciaInvertida import metodoPotenciaInvertida


def ventanaPotenciaInvertida():
    sub = tk.Toplevel()
    sub.title("Método de la Potencia Invertida")
    sub.geometry("600x600")

    tk.Label(sub, text="Método de la Potencia Invertida",
    ...
    def resolver():
        try:
            ...
            λ, v, it = metodoPotenciaInvertida(A)

            texto = f"Autovalor más pequeño (aprox):\nλ = {λ:.6f}\n\n"
            texto += "Autovector asociado:\n"
            texto += str(v) + "\n"
            texto += f"\nIteraciones: {it}"
